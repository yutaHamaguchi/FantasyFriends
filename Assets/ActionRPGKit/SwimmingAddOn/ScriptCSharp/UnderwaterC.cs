	 ...
	 IEnumerator ActivateWaterController(){
 		if(!onEnter){
 			if(player.GetComponent<UnderwaterControllerC>()){
				//Get Off  the Mount when Going under water.
				if(player.GetComponent<AttackTriggerC>().actvateObj){
					GameObject mm = player.GetComponent<AttackTriggerC>().actvateObj;
					if(mm.GetComponent<MountControllerC>()){
						mm.GetComponent<MountControllerC>().Deactivate();
					}
					player.GetComponent<AttackTriggerC>().RemoveActivator(mm);
				}
 				onEnter = true;
