	...
	void Update(){
		...
		if(followState == AIStatef.FollowMaster){
			...
			if((master.position - transform.position).magnitude <= 3.0f){
				...
				if((master.position - transform.position).magnitude > 5.5f){
					controller.Move(forward * sprintSpeed * Time.deltaTime);
				}else{
			...
			if (masterDistance > detectRange + 5.0f){//////////////////GetMasterPosition
				...
				Vector3 forward = transform.TransformDirection(Vector3.forward);
				if((followTarget.position - transform.position).magnitude >= approachDistance + 2.5f){
					controller.Move(forward * sprintSpeed * Time.deltaTime);
				}else{
					controller.Move(forward * speed * Time.deltaTime);
				}
