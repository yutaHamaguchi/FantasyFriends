...
public class ShopCanvasC : MonoBehaviour{
	...
	public void ShowTooltip(int slot){
		...
		if(mode == 1){
			if(slot >= player.GetComponent<InventoryC>().itemSlot.Length || player.GetComponent<InventoryC>().itemSlot[slot] <= 0){
				HideTooltip();
				return;
			}
		...
		if(mode == 2){
			if(slot >= player.GetComponent<InventoryC>().equipment.Length || player.GetComponent<InventoryC>().equipment[slot] <= 0){
				HideTooltip();
				return;
			}
	...
	public void ButtonClick(int slot){
		...
		if(mode == 0){
			if(pickupSlot >= shopSlot.Length || shopSlot[pickupSlot].itemId <= 0){
				return;
			}
		...
		if(mode == 1){
			if(pickupSlot >= player.GetComponent<InventoryC>().itemSlot.Length || player.GetComponent<InventoryC>().itemSlot[pickupSlot] <= 0){
				return;
			}
			shopPanel.SetActive(false);
		...
		if(mode == 2){
			if(pickupSlot >= player.GetComponent<InventoryC>().equipment.Length || player.GetComponent<InventoryC>().equipment[pickupSlot] <= 0){
				return;
			}
			shopPanel.SetActive(false);
			sellConfirmation.basePanel.SetActive(true);
